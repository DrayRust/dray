...
#[tauri::command]
async fn ping_test(url: String, count: usize, timeout: u64) -> Value {
    http::ping_test(&url, count, timeout).await
}
...
#[tauri::command]
async fn download_speed_test(url: String, timeout: u64) -> Value {
    http::download_speed_test(&url, timeout).await
}
